  ...
  blog: {
    ...
    getComments: async (postId) => {
      try {
        ...
        if (error && (error as any).code === "PGRST116") {
          console.warn(
            "[blog.getComments] Table missing, returning empty list",
          );
      ...
      try {
        ...
        if (error && (error as any).code === "PGRST116") {
          return formatResponse(
            ...
            {
              message:
                "Comments feature not configured: table blog_comments missing",
            },
            "blog.createComment",
          );
        }
    ...
    getCategories: async () => {
      try {
        ...
        if (error && (error as any).code === "PGRST116") {
          console.warn(
            "[blog.getCategories] Table missing, returning empty list",
          );
