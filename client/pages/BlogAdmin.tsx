    ...
    try {
      ...
      if (activeTab === 'posts') {
        if (editingItem) {
          ...
          // Create flow - generate slug from title
          ...
          const dataToCreate = {
            ...
            author_name: 'Nolads Engineering',
            author_email: 'engineering@nolads.com'
          };

          const createRes = await api.blog.create(dataToCreate);
        ...
        if (editingItem) {
          response = await api.blog.updateCategory((editingItem as any).id, categoryData);
        } else {
          response = await api.blog.createCategory(categoryData);
        }
      }
